...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--serials",nargs='+', dest="serials", required=True,
                        help="Kinect Serials", metavar="SERIALS")
   ...
   # for key, value in sorted(vars(args).items()):
    ...
    #        exrFile = imgFile.split('.')[0] + ".exr"
        ...
        trans= np.vstack([np.hstack((ret_R,ret_t)),[0,0,0,1]])
        transOutputFile=""
        for increment,arg in enumerate([serialsList[k], serialsList[k+1]]):
